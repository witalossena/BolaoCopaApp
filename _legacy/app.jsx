/* ============================================================
   APP — estado, rotas e persistência
   ============================================================ */
...
function App() {
  const saved = React.useRef(loadStore()).current;

  const [view, setView]         = React.useState(saved.view || "landing");
  const [user, setUser]         = React.useState(saved.user || null);
  const [scores, setScores]     = React.useState(saved.scores || {});
  const [ranks, setRanks]       = React.useState(saved.ranks || {});
  const [specials, setSpecials] = React.useState(saved.specials || {});
  const [adminUsers, setAdminUsers] = React.useState(saved.adminUsers || null);
  ...
  if (!user) {
    ...
    if (view === "pub_ranking")
      return <PublicShell active="pub_ranking" go={setView}><Ranking ranking={buildRanking(MOCK_USERS)} currentUser={{ handle: "__none__" }} /></PublicShell>;
    if (view === "pub_regras")
      return <PublicShell active="pub_regras" go={setView}><Regras /></PublicShell>;
